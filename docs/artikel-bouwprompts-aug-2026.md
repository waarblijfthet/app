...
| 17 aug 2026 | Klus 10, situatieparameters in CTA's (sessie 1) | Alle artikelen in `lib/inzichten-data.ts` doorgelopen (in het logboek stond 72, het waren er 83; gecorrigeerd 18-aug-2026); slechts 13 hebben uberhaupt een `cta`-veld (de rest heeft geen CTA-blok, zie `Artikel.cta?`, dus geen `primairHref` om te converteren, bijvoorbeeld `goed-salaris-toch-krap`, de winnaar uit klus 2, ondanks dat dit vermoedelijk veel verkeer trekt). Van die 13 waren er 4 al klaar (situatieparameter of `/analyse` als primair doel: `huishoudboekje-voorbeeld`, `niet-rondkomen-met-4000-euro-netto`, `alleen-wonen-goed-salaris-toch-krap`, `50-30-20-regel-hoger-inkomen`). Van de resterende 9 met kale `primairHref: "/geldscan"` zijn er 3 omgezet, de andere 6 bewust ONGEWIJZIGD gelaten omdat de inhoud meerdere huishoudtypen gelijkwaardig behandelt of geen huishoudtype noemt (te vermijden gokken, zoals de opdracht vroeg): `klarna-niet-kunnen-betalen` (geen huishoudinvalshoek), `netto-loonsverhoging-berekenen` (geen huishoudinvalshoek), `wat-is-normaal-bedrag-boodschappen-per-maand` (bedient expliciet alleenstaand, stel, gezin, alleenstaande ouder en zzp naast elkaar in een tabel), `is-5000-euro-netto-goed-salaris` (kernbetoog is juist eenverdiener-versus-tweeverdieners, gebruikt zowel het stel-zonder-kinderen- als het gezinsrapport), `samen-6000-euro-netto-toch-niets-over` (twee gelijkwaardige cases: stel zonder kinderen en gezin met drie kinderen), `is-4000-euro-netto-goed-salaris-nederland` (de plek-1-pillar, `SalarisRekenaar` zonder startwaarden, bewust universeel voor alle huishoudtypen). Omgezet: `tweeverdieners-toch-krap` naar `/geldscan?situatie=gezin` (content draait om de kinderopvang-val en linkt naar de potjesmethode-gezin en BSO-kosten, dus gezinnen met kinderen); `nibud-boodschappen-versus-werkelijkheid` naar `/geldscan?situatie=gezin` (elk voorbeeld en elke norm in de tekst is een gezin, geen enkele vermelding van alleenstaand, stel of zzp); `kosten-levensonderhoud-alleenstaande-2026` naar `/geldscan?situatie=alleenstaand` (titel en inhoud zijn expliciet en uitsluitend over de alleenstaande). Terzijde gevonden en meegecommit: een al langer bestaande syntaxfout in `lib/inzichten-data.ts` die sinds klus 9 onopgemerkt bleef: de `const HB_STEL = rapportVoorSlug(...)`-declaratie stond na de opening `[` van `export const artikelen: Artikel[] = [`, dus binnen de array-literal in plaats van ervoor. Dat brak `npx tsc --noEmit --incremental false` voor het HELE bestand (1546 foutregels, cascade vanaf regel 180), ook al op de vorige commit (bevestigd door `git show HEAD:lib/inzichten-data.ts` los te testen). Gefixt door de const-declaratie en zijn JSDoc-comment voor `export const artikelen` te plaatsen. `npx tsc --noEmit --incremental false` is nu schoon (exit 0), geen null bytes. Geen nieuwe URL's, geen GSC-indiening nodig. Nog 6 kandidaten met kale `/geldscan` open voor een volgende sessie plus alle 59 artikelen zonder `cta`-veld (die zouden een heel CTA-blok met kop, tekst en labels nodig hebben, buiten de scope van dit mechanische klusje). | Geen toetsronde nodig per opdracht (mechanisch werk). Zelfcontrole: elke conversie getoetst aan de content-component van het artikel zelf (kinderen, gezin en alleenstaand-vermeldingen opgezocht met grep), niet geraden. `npx tsc --noEmit --incremental false` schoon, geen null bytes, `wc -l` ongewijzigd op 4519 regels. |
