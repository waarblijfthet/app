...
**Update 7 september, buiten deze volgorde om, twee opdrachten.** Op expliciet en gedetailleerd verzoek van Jarno herschreven: `app/analyse/IntroScherm.tsx`, het scherm vóór "Start mijn analyse". Eerste opdracht: inhoud en UX binnen de bestaande smalle kolom. Tweede opdracht, dezelfde dag: omgebouwd tot volwaardige brede landingpage (max 1180px, twee-koloms hero, resultaatpreview, kaarten), met een kleine wrapperwijziging in `QuizClient.tsx` om buiten de smalle 600px-kolom te renderen. Geen bouwvolgorde-item, geen contentpagina, dus telt niet mee in de tempo-regel van sectie 1. Zie sectie 17 en 18. Ongepusht, hoort bij dezelfde push als de rest van deze sectie.
...
8. **IntroScherm /analyse herschreven (7-sep-2026), buiten de bouwvolgorde om, op verzoek van Jarno, in twee opdrachten dezelfde dag.** Zie sectie 17 (eerste opdracht) en 18 (tweede opdracht: brede landingpage, ook `QuizClient.tsx` geraakt voor de bredere wrapper). Geen productiebuild gedraaid: `next dev` en `next build` sterven op de mount zelf, precies zoals `feedback_minifier_verkorte_objectnotatie` beschrijft (geen SWC-binary, geen netwerk), en het bestand bevat geen berekende objecten, dus niet het specifieke minifier-risicopatroon uit die regel; `tsc` is wel schoon op beide bestanden. **Loop de pagina zelf even langs op telefoon en desktop na de push, dit is de eerste keer dat dit scherm in een echte browser gezien wordt.**
