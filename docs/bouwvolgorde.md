...
**Update 7 september, buiten deze volgorde om.** Op expliciet en gedetailleerd verzoek van Jarno herschreven: `app/analyse/IntroScherm.tsx`, het scherm vóór "Start mijn analyse". Geen bouwvolgorde-item, geen contentpagina, dus telt niet mee in de tempo-regel van sectie 1. Zie sectie 17. Ongepusht, hoort bij dezelfde push als de rest van deze sectie.
...
8. **IntroScherm /analyse herschreven (7-sep-2026), buiten de bouwvolgorde om, op verzoek van Jarno.** Zie sectie 17. Geen productiebuild gedraaid: het bestand bevat geen berekende objecten, dus niet het minifier-risicopatroon uit `feedback_minifier_verkorte_objectnotatie`; `tsc` is wel schoon. **Loop de pagina zelf even langs op telefoon en desktop na de push**, dit is een groot visueel blok dat niet in een echte browser gezien is.
