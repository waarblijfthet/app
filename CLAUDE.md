...
- Route, label en situatieparameters komen uit `lib/cta.ts`, nooit met de hand overgetypt.
...
- Elke CTA loopt via `components/CtaLink.tsx`, zodat `cta_analysis` en `cta_geldscan` apart geteld worden met pagina, locatie en vervolgpagina. De site linkt nergens rechtstreeks naar `/aanbod/intake?pakket=geldscan`; die intake blijft werken en is bereikbaar vanaf `/geldscan` en vanaf de resultaatpagina.
