...
- Route, label en parameters komen uit `lib/cta.ts`: `analyseHref({ situatie, inkomen, boodschappen })`, `GELDSCAN_ROUTE`, `GELDSCAN_CTA_LABEL`, `geldscanHref({ token, keuze })`. Typ `/aanbod/intake?pakket=geldscan` nergens uit. Elke Geldscan-link gaat rechtstreeks naar het aanvraagformulier, niet naar /geldscan; /geldscan blijft bestaan voor zoekverkeer.
...
- **Tweede ingang, besluit van Jarno op 25-sep-2026 (wijkt bewust af van de pakketregel hieronder).** Naast "waar blijft mijn geld" is er een tweede ingang naar dezelfde Geldscan: doorrekening vóór een keuze (minder werken, een ander huis, uit elkaar gaan, een (tweede) kind, een kind dat 18 wordt). Geen nieuw product, geen nieuwe prijs, geen nieuwe levertijd, geen nieuwe ladder. De vijf keuzes staan alleen in `lib/geldmomenten.ts`. Het formulier heeft één optioneel keuzeveld (`?keuze=` vult het voor); de keuze gaat als `[Keuze: <label>]` vóór het bericht in `grootste_knelpunt`, zonder nieuwe kolom. Het rapport krijgt dan het blok "Nu tegenover na je keuze", dat Jarno zelf schrijft (`docs/vragenlijst-keuze-aanvulling.md`). Een artikel over een keuze krijgt `geldscanKeuze` in `lib/inzichten-data.ts`; dat stuurt de slotzin en de keuze op de ene Geldscan-link in het slotblok.
- **De gratis doorrekening mag niet te goed worden.** `components/artikel/KeuzeDoorrekening.tsx` beantwoordt alleen "wat verandert er ongeveer aan onze maand". Enkele veranderende posten, vrije ruimte nu en na, het verschil. Nooit een volledige begroting, nooit alle uitgavencategorieën, nooit "je kunt dit wel of niet betalen", geen bruto-netto, hypotheek-, alimentatie- of pensioenberekening. Onder de uitkomst: indicatie, dan de brug ("of dit past hangt af van waar jullie geld nu al naartoe gaat"), dan de analyse-CTA. Nooit een Geldscan-link vanuit de doorrekening.
