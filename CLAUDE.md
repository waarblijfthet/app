...
- Ladder: gratis analyse → Geldscan 49 euro (aanvraagroute `geldscanHref()`, uitlegpagina /geldscan, in copy Geldrapport) → adviesgesprek 125 euro → traject 497 euro. De laatste twee blijven tekst op aanvraag, nooit een prijskaart naast het rapport.
...
- Route, label en situatieparameters komen uit `lib/cta.ts`, nooit met de hand overgetypt. Voor de Geldscan zijn dat `GELDSCAN_ROUTE`, `GELDSCAN_CTA_LABEL` en `geldscanHref({ token })`. Typ `/aanbod/intake?pakket=geldscan` nergens uit, dan blijft er precies één plek waar de route staat.
- **Per artikel precies één Geldscan-verwijzing, en die staat in het slotblok van `app/inzichten/[slug]/page.tsx`.** Zet er nooit een tweede in de artikeltekst of in een rekenaar, dat maakt de route artikel, analyse, Geldscan, analyse, Geldscan. Maximaal twee prominente analyse-CTA's per artikel, het slotblok is er altijd één van.
- Rekenaars zijn een opstap, geen eindpunt: onder elke uitkomst staat dat het een indicatie is, daarna de analyse-CTA, en nooit een Geldscan-link.
...
- **Sinds 30-aug-2026 gaat elke Geldscan-link rechtstreeks naar het aanvraagformulier**, dus naar `geldscanHref()` en niet meer naar `/geldscan`. Reden: wie op /aanbod of onder een artikel al gelezen had wat de Geldscan is, kreeg datzelfde verhaal op /geldscan een tweede keer voordat hij iets kon aanvragen. Dat is de oude regel "de site linkt nergens rechtstreeks naar de intake" die hiermee vervalt. `/geldscan` blijft bestaan als uitlegpagina voor zoekverkeer, maar geen enkele knop op de site stuurt er nog heen.
- **Het aanvraagformulier vraagt vóór de betaling alleen voornaam en e-mailadres**, plus een optionele situatie en een optioneel bericht van 300 tekens. De pagina staat in `app/aanbod/intake/GeldscanAanvraag.tsx`. Zet daar nooit inkomen, woonlasten, uitgaven, auto, hypotheek of bankafschriften in: dat is een intake voor iets wat nog niet gekocht is. Die gegevens vraagt Jarno met de hand op na betaling. Voorinvulling vanuit de analyse is daarmee ook verdwenen; alleen het analysetoken reist nog mee.
